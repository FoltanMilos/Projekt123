    ...
    for img_path in glob.iglob("dataset/cnn/processed/benign/*.jpg"):
        with open(img_path, 'rb') as file:
            ...
            if(ratio_index <= conf.count_train_benign):
                with open(path + '/train/benign/'+img_path.split("\\")[1] , 'wb') as newfile:
                ...
                with open(path + '/validation/benign/' + img_path.split("\\")[1], 'wb') as newfile:
                ...
                with open(path + '/test/benign/' + img_path.split("\\")[1], 'wb') as newfile:
                    ...
                    newfile.close()
            file.close()
        print("["+str(p_index)+"/"+ str(print_max)+"] - " + img_path.split("\\")[1] + " -> " + print_path)
    ...
    for img_path in glob.iglob("dataset/cnn/processed/malignant/*.jpg"):
        with open(img_path, 'rb') as file:
            ...
            if(ratio_index <= conf.count_train_malig):
                with open(path + '/train/malignant/'+img_path.split("\\")[1] , 'wb') as newfile:
                ...
                with open(path + '/validation/malignant/' + img_path.split("\\")[1], 'wb') as newfile:
